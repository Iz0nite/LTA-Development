<?php
	...
	$pdf->Cell(55, 8, $invoceText, 0, 0, 'L');
	
	// date facture
	$input = $tab[1]['creationDate'];
	$date = strtotime($input);
	$date_fact = date('d/m/Y', $date);
	...
	$pdf->Cell(140, 8, "Label", 0, 0, 'C');
	$pdf->SetXY(145, 96);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(13, 8, "quantity", 0, 0, 'C');
	$pdf->SetXY(156, 96);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(22, 8, "Price HT", 0, 0, 'C');
	$pdf->SetXY(177, 96);
	$pdf->SetFont('Arial', 'B', 8);
	...
	//lignes horizontales haut de cadre
	$pdf->Line(145, 95, 145, 105);
	$pdf->Line(158, 95, 158, 105);
	$pdf->Line(176, 95, 176, 105);
	$pdf->Line(187, 95, 187, 105);
	...
	foreach($tab[0] as $package){
		...
		if($counter % 26 == 0){	//modulo will determinate how much packages will be showed per page
			$pdf->SetLineWidth(0.1);
			$pdf->Rect(5, 95, 200, $y-95, "D");
		...
		// libelle
		$pdf->SetXY(7, $y);
		$pdf->Cell(140, 5, "Package number " . $counter, 0, 0, 'L');
		...
		$pdf->Line(145, $hautCorpsCadre, 145, $hautCorpsCadre + 7);
		$pdf->Line(158, $hautCorpsCadre, 158, $hautCorpsCadre + 7);
		$pdf->Line(176, $hautCorpsCadre, 176, $hautCorpsCadre + 7);
		$pdf->Line(187, $hautCorpsCadre, 187, $hautCorpsCadre + 7);
	...
	//cadre de corps page dernière page
	$pdf->SetLineWidth(0.1);
	$pdf->Rect(5, 95, 200, $y-95.7, "D");
...
//	$pdf->Rect(5, 95, 200, $y - 95, "D");
	...
	$pdf->SetFont('Arial', '', 9);
	$pdf->Cell(60, 5, round($totalTax, 2), 0, 1, 'L');
	...
	$pdf->SetFont('Arial', '', 9);
	$pdf->Cell(60, 5, round($totalPrice, 2), 0, 1, 'L');
